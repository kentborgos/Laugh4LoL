...
export function jokesterUnlocked() {
  try {
...
export function requireJokester() {
  if (!jokesterUnlocked()) throw new Error("Jokester is locked.");
}
...
export function unlockJokesterWithPassword(password: string) {
  ...
  setCookie(COOKIE, tokenForHash(got), {
    path: "/",
    httpOnly: true,
    sameSite: "lax",
    maxAge: 60 * 60 * 24 * 7,
